 ...
 */
package es.domin.clientes;

import es.domin.db.DatosCliente;
...
public class Main extends HttpServlet {
    ...
    private static final Logger logger = Logger.getLogger(DatosCliente.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        DatosCliente.connect(dbServer, dbName, dbUser, dbPassword);

        //Comprueba si se ha podido realizar la conexión
        if (DatosCliente.isConnected()) {
            ...
            if (action != null && !action.isEmpty()) {
                ...
                if (action.equals(ACTION_EDIT_REQUEST)) {
                    ...
                    Cliente cliente = DatosCliente.getClienteByID(id);
                    ...
                    redirectTo("cliente_detail.jsp", request, response);
                    ...
                    updateClienteWithRequestData(cliente, request);
                    //Se actualiza en la BD la persona
                    DatosCliente.updateCliente(cliente);
                    ...
                    Cliente cliente = new Cliente();
                    updateClienteWithRequestData(cliente, request);
                    DatosCliente.insertCliente(cliente);
                    response.sendRedirect("Main?action=");
                    ...
                    DatosCliente.deleteClienteById(id);
                    response.sendRedirect("Main?action=");
                    ...
                    ArrayList<Cliente> clienteList = DatosCliente.getClienteList();
                ...
                ArrayList<Cliente> clienteList = DatosCliente.getClienteList();
                request.setAttribute("clienteList", clienteList);
                redirectTo("cliente_list.jsp", request, response);
            }
        } else { //No se ha podido hacer la conexión con la BD
            response.setContentType("text/html;charset=UTF-8");
            PrintWriter out = response.getWriter();
            out.println("Error: Conexion fallida bd.<br>");
     ...
     * @param request
     */
    private void updateClienteWithRequestData(Cliente cliente, HttpServletRequest request) {
        cliente.setName(request.getParameter("nombre"));
        cliente.setSurnames(request.getParameter("apellidos"));
        cliente.setPhoneNumber(request.getParameter("numtelefono"));
        cliente.setAddress(request.getParameter("direccion"));
        cliente.setCity(request.getParameter("ciudad"));
        ...
        if (!request.getParameter("birth_date").isEmpty()) {
            try {
                ...
                cliente.setBirthDate(birthDate.getTime());
        ...
        if(request.getParameter("deletePhoto")!=null && request.getParameter("deletePhoto").equals("on")) {
            ...
            if(!cliente.getPhotoFileName().isEmpty() && cliente.getPhotoFileName()!=null) {
                deletePhotoFile(request, cliente.getPhotoFileName());
                cliente.setPhotoFileName("");
            }
            ...
            if(photoFileName!=null) {
                if(cliente.getPhotoFileName().isEmpty() || cliente.getPhotoFileName()==null) {
                    ...
                    cliente.setPhotoFileName(photoFileName);
                } else {
